...
class FrameExtractor:
    ...
    def run(self):
        ...
        with utils.ParallelProgressBar(n_jobs=self.num_jobs) as parallel:
            parallel.tqdm(
                desc="Extracting frames from frame folders", unit=" clips"
            )
            frame_paths = parallel(self._clip_worker, clip_paths)
...
@hydra.main(config_path="configs", config_name="extract_frame_dirs")

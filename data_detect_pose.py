...
class PoseClipMaker:
    ...
    def __post_init__(self):
        ...
        self.clip_frames_db = data.ImageDatabase(
            clip_frames_db_path, readonly=False, quality=self.quality
        )
    ...
    def run(self):
        ...
        for i, frame_key in enumerate(progress_bar):
            ...
            if (
                ...
                or frame_index - prev_frame_index != 1
            ):
    ...
    def _clip_worker(self, index: int, clip: str):
        ...
        for frame_index, frame_key in enumerate(clip):
            ...
            frame_name = f"{clip_name}_frame_{frame_index:05d}"
...
@hydra.main(config_path="configs", config_name="detect_pose")
def filter_people(config: omegaconf.DictConfig):
    ...
    if is_rank_zero():
        clip_maker = PoseClipMaker(
            input_dir, output_dir, config.min_frames, config.quality
        )
        clip_maker.run()
